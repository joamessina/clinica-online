...
export class MisTurnosPacienteComponent implements OnInit {
  ...
  async ngOnInit() { this.turnos.set(await this.svc.listPatient()); }
  ...
  canCancel(t:Turno){ return ['PENDIENTE','ACEPTADO'].includes(t.estado); }
  canFeedback(t:Turno){ return t.estado==='REALIZADO'; }
  canSurvey(t:Turno){ return t.estado==='REALIZADO' && !!t.resena_especialista; }
  ...
  async feedback(t: Turno) {
    ...
    const { error } = await this.svc.sendFeedback(t.id, cal, comentario ?? undefined);
